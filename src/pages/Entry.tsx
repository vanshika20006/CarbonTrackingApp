...
export default function Entry() {
  ...
  // 🌱 NEW STATE (TREE OFFSET FEATURE)
  const [lastEmission, setLastEmission] = useState<number | null>(null);
  const [treesRequired, setTreesRequired] = useState<number | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 🌱 TREE CALCULATION
      const trees = calculateTreesRequired(emission);
      setLastEmission(emission);
      ...
      if (isDemoMode) {
        ...
        await supabase.from('carbon_entries').insert({
          user_id: user.id,
      ...
      toast({
        title: '✅ Entry Saved',
        description: `Emission: ${emission.toFixed(2)} kg CO₂`,
      });
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8 max-w-2xl">
       ...
       <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* 🌱 TREE OFFSET RESULT CARD */}
        {lastEmission !== null && treesRequired !== null && (
          <Card className="mt-6 border-emerald-500/40 bg-emerald-500/5">
            <CardHeader>
              <CardTitle className="text-emerald-600">
                🌍 Carbon Neutralization Impact
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              <div className="flex justify-between">
                <span>Total CO₂ Emitted</span>
                <span className="font-semibold">{lastEmission.toFixed(2)} kg</span>
              </div>
              <div className="flex justify-between">
